...
import com.minatogithuborganization.appengine.base.model.Table123Base;
import java.util.List;
...
public class Table123BaseService<T extends Table123Base> extends BaseBusinessLogic<T>
		implements ITable123BaseService<T> {
	...
	@Autowired
	private  Table123PerimeterBaseImpl table123PerimeterBaseImpl;
	
	public Table123BaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("Table123", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("Table123", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("Table123", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return table123PerimeterBaseImpl;
	}
}